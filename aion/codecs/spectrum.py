...
class AutoencoderSpectrumCodec(Codec):
    ...
    def forward(
        ...
        与 train_spectrum_codec_v2.py 中的调用方式兼容：
        outputs = model(flux, ivar, mask, wavelength)
        
        Returns:
            dict with keys:
                - flux_pred: reconstructed flux
                - mask_logits: predicted mask logits
                - commitment_loss: quantizer commitment loss
        """
        # 构造 Spectrum 对象来调用 _encode
        from aion.modalities import Spectrum as SpectrumModality
        x = SpectrumModality(flux=flux, ivar=ivar, mask=mask, wavelength=wavelength)
        
        # 编码
        h, normalization = self._encode(x)
        
        # 量化 (训练模式下返回损失)
        z_q, commitment_loss, diversity_loss = self.quantizer(h)
        aux_loss = commitment_loss + diversity_loss
        
        # 解码（不返回 Spectrum 对象，而是返回原始 tensor）
        h_dec = self.post_quant_conv(z_q)
        spectra = self.decoder(h_dec)

        if spectra.shape[1] == 1:
            spectra = spectra.squeeze(1)
            mask_logits = torch.zeros_like(spectra)
        elif spectra.shape[1] == 2:
            spectra, mask_logits = spectra.chunk(2, dim=1)
            spectra, mask_logits = spectra.squeeze(1), mask_logits.squeeze(1)
        else:
            raise ValueError("Invalid number of output channels, must be 1 or 2")

        # 插值到观测波长网格
        spectra = self.latent_grid.to_observed(spectra, wavelength)
        mask_logits = self.latent_grid.to_observed(mask_logits, wavelength)

        # 应用归一化
        flux_pred = (spectra + 1.0) * torch.clamp(
            10 ** normalization[..., None] - 1.0, 0.1
        )
