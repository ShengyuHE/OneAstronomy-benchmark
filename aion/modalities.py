...
@dataclass
class LightCurve(Modality):
    """Base class for light curve modality data.

    This is an abstract base class. Use LightCurveM3 instead.
    """

    name: ClassVar[str] = "lightcurve"
    flux: Float[Tensor, " batch length"]
    delta_t: Float[Tensor, " batch length"]
    delta_t_interval: Float[Tensor, " batch length"]
    error: Float[Tensor, " batch length"] | None = None
    context_mask: Bool[Tensor, " batch length"] | None = None

    def __repr__(self) -> str:
        repr_str = (
            f"LightCurve(flux_shape={list(self.flux.shape)}, "
            f"length={self.flux.shape[-1]})"
        )
        return repr_str


class M3LightCurve(LightCurve):
    """Light curve modality data for AstroM3 Phase 2 model."""

    token_key: ClassVar[str] = "tok_lightcurve_m3"
    num_tokens: ClassVar[int] = 13

class TESSLightCurve(LightCurve):
    """Light curve modality data for AstroTESS Phase 2 model."""

    token_key: ClassVar[str] = "tok_lightcurve_tess"
    num_tokens: ClassVar[int] = 13
